  ...
  $(".search-form").submit(() => {
    ...
    $.ajax("/allTodos", {
      ...
      for (todo of allTodos[1]) {
        ...
        const $divLeftBottom = $(`<div>`).addClass(`left-bottom`);
        const $divLeftBottomBtn = $(`<a>`).addClass(`url`).attr({ href: `https://imdb.com/title/` + todo.imdbID, target: '_blank' }).text('IMDB');
      ...
      for (todo of allTodos[2]) {
        ...
        if (todo.volumeInfo.imageLinks) {
          $divLeftImg = $('<img>').addClass('image circle').attr('src', todo.volumeInfo.imageLinks.thumbnail);
        } else {
          $divLeftImg = $('<img>').addClass('image circle').text("NO IMAGE");
        }
        const $divLeftBottom = $(`<div>`).addClass(`left-bottom`);
      ...
      for (todo of allTodos[4]) {
        ...
        if (todo.images.length !== 0) {
          $divLeftImg = $('<img>').addClass('image circle').attr('src', todo.images[0].url);
        } else {
          $divLeftImg = $('<img>').addClass('image circle').text("NO IMAGE");
        }
        const $divLeftBottom = $(`<div>`).addClass(`left-bottom`);
